...
local Section = Tab:AddSection({
    Name = "وەک پشگیری سکریپتەکە بو هاورێتان بنێرن. فولوی تیک توک \ntiktok : x6iu1"
})
...
local function onChatted(p,msg)
    if _G.chatSpyInstance == instance then
        if p==player and msg:lower():sub(1,4)=="/spy" then
            enabled = not enabled
            wait(0.3)
            privateProperties.Text = "{ESTA "..(enabled and "PRV" or "").."ABENE✅}"
        ...
        elseif enabled and (spyOnMyself==true or p~=player) then
            ...
            if hidden and enabled then
                if public then
                    saymsg:FireServer((publicItalics and "/me " or '').."{PRIV🔐} [".. p.Name .."]: "..msg,"All")
                else
...
privateProperties.Text = "{ESTA "..(enabled and "PRIV" or "").."ABENE✅}"
StarterGui:SetCore("ChatMakeSystemMessage",privateProperties)
